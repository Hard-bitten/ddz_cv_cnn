...
class char_cnn_net:
    def __init__(self):
        self.dataset = numbers + alphbets + shape + other
...
if __name__ == '__main__':
    cur_dir = sys.path[0]
    data_dir = os.path.join(cur_dir, 'pic/train')
    test_dir = os.path.join(cur_dir, 'pic/test')
    train_model_path = os.path.join(cur_dir, 'model/char_recongnize/model.ckpt')
    model_path = os.path.join(cur_dir, 'model/char_recongnize/model.ckpt-520')
